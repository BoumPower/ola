...
const char I2CBackendInterface::I2C_DROP_VAR[] = "i2c-drops";
const char I2CBackendInterface::I2C_DROP_VAR_KEY[] = "device";
...
HardwareBackend::HardwareBackend(const Options &options,
  ...
  if (export_map) {
    m_drop_map = export_map->GetUIntMapVar(I2C_DROP_VAR,
                                           I2C_DROP_VAR_KEY);
    (*m_drop_map)[m_i2c_writer->DevicePath()] = 0;
  }
}
...
bool HardwareBackend::Init() {
  if (!(m_i2c_writer->Init() && SetupGPIO())) {
    return false;
  }
...
void HardwareBackend::Commit(uint8_t output) {
  ...
  if (output_data->IsPending() && m_drop_map) {
    ...
    (*m_drop_map)[m_i2c_writer->DevicePath()]++;
  }
...
void HardwareBackend::WriteOutput(uint8_t output_id, OutputData *output) {
  ...
  for (unsigned int i = 0; i < m_gpio_fds.size(); i++) {
    ...
    if (m_gpio_pin_state[i] != pin) {
      ...
      if (write(m_gpio_fds[i], data.c_str(), data.size()) < 0) {
        OLA_WARN << "Failed to toggle I2C GPIO pin "
  ...
  m_i2c_writer->WriteI2CData(output->GetData(), output->Size());
}
...
uint8_t *SoftwareBackend::Checkout(uint8_t output,
  ...
  if (output >= m_output_sizes.size()) {
    OLA_WARN << "Invalid I2C output " << static_cast<int>(output);
    return NULL;
  }
...
void *SoftwareBackend::Run() {
  ...
  while (true) {
    ...
    if (write_pending) {
      m_i2c_writer->WriteI2CData(output_data, length);
    }
  }
}

FakeI2CBackend::FakeI2CBackend(unsigned int outputs) {
  for (unsigned int i = 0; i < outputs; i++) {
    m_outputs.push_back(new Output());
  }
}

FakeI2CBackend::~FakeI2CBackend() {
  STLDeleteElements(&m_outputs);
}

uint8_t *FakeI2CBackend::Checkout(uint8_t output_id,
                                  unsigned int length,
                                  unsigned int latch_bytes) {
...
void FakeI2CBackend::Commit(uint8_t output) {
...
const uint8_t *FakeI2CBackend::GetData(uint8_t output_id,
                                       unsigned int *length) {
  if (output_id >= m_outputs.size()) {
    return NULL;
  }
...
unsigned int FakeI2CBackend::Writes(uint8_t output) const {
  if (output >= m_outputs.size()) {
    return 0;
  }
