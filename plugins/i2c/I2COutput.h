...
#ifndef PLUGINS_I2C_I2COUTPUT_H_
#define PLUGINS_I2C_I2COUTPUT_H_
...
class I2COutput: public ola::rdm::DiscoverableRDMControllerInterface {
 public:
  // definitions for all I2C Personalities
  ...
  // I2COutput.cpp I2COutput::GetDeviceInfo()
  enum I2C_PERSONALITY {
  ...
  struct Options {
    ...
    explicit Options(uint8_t output_number, const std::string &i2c_device_name)
        : device_label("I2C Device - " + i2c_device_name),
          pixel_count(25),  // For the https://www.adafruit.com/products/738
          output_number(output_number) {
    }
  };

  I2COutput(const ola::rdm::UID &uid,
            class I2CBackendInterface *backend,
 ...
 private:
   ...
   */
  class RDMOps : public ola::rdm::ResponderOps<I2COutput> {
   public:
   ...
   private:
    RDMOps() : ola::rdm::ResponderOps<I2COutput>(PARAM_HANDLERS) {}

    static RDMOps *instance;
  };

  class I2CBackendInterface *m_backend;
  ...
  void WriteI2CData(const uint8_t *data, unsigned int length);
  ...
  static const uint16_t I2C_DELAY;
