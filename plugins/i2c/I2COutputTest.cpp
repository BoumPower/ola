...
using ola::DmxBuffer;
using ola::plugin::i2c::FakeI2CBackend;
using ola::plugin::i2c::I2CBackendInterface;
using ola::plugin::i2c::I2COutput;
using ola::rdm::UID;
using std::string;

class I2COutputTest: public CppUnit::TestFixture {
  CPPUNIT_TEST_SUITE(I2COutputTest);
...
CPPUNIT_TEST_SUITE_REGISTRATION(I2COutputTest);

void I2COutputTest::setUp() {
 ...
 */
void I2COutputTest::testDescription() {
  FakeI2CBackend backend(2);
  I2COutput output1(m_uid, &backend, I2COutput::Options(0, "Test I2C Device"));

  I2COutput::Options options(1, "Test I2C Device");
  options.pixel_count = 32;
  I2COutput output2(m_uid, &backend, options);
  ...
  OLA_ASSERT_EQ(
    static_cast<uint8_t>(I2COutput::PERS_WS2801_INDIVIDUAL),
  ...
  output1.SetPersonality(I2COutput::PERS_LDP8806_INDIVIDUAL);
  OLA_ASSERT_EQ(
  ...
  OLA_ASSERT_EQ(
    static_cast<uint8_t>(I2COutput::PERS_LDP8806_INDIVIDUAL),
    output1.GetPersonality());
  // Test for backwards compatibility
 ...
 */
void I2COutputTest::testIndividualWS2801Control() {
  FakeI2CBackend backend(2);
  I2COutput::Options options(0, "Test I2C Device");
  options.pixel_count = 2;
  I2COutput output(m_uid, &backend, options);
  output.SetPersonality(I2COutput::PERS_WS2801_INDIVIDUAL);
 ...
 */
void I2COutputTest::testCombinedWS2801Control() {
  FakeI2CBackend backend(2);
  I2COutput::Options options(0, "Test I2C Device");
  options.pixel_count = 2;
  I2COutput output(m_uid, &backend, options);
  output.SetPersonality(I2COutput::PERS_WS2801_COMBINED);
 ...
 */
void I2COutputTest::testIndividualLPD8806Control() {
  FakeI2CBackend backend(2);
  I2COutput::Options options(0, "Test I2C Device");
  options.pixel_count = 2;
  I2COutput output(m_uid, &backend, options);
  output.SetPersonality(I2COutput::PERS_LDP8806_INDIVIDUAL);
 ...
 */
void I2COutputTest::testCombinedLPD8806Control() {
  FakeI2CBackend backend(2);
  I2COutput::Options options(0, "Test I2C Device");
  options.pixel_count = 2;
  I2COutput output(m_uid, &backend, options);
  output.SetPersonality(I2COutput::PERS_LDP8806_COMBINED);
 ...
 */
void I2COutputTest::testIndividualP9813Control() {
  FakeI2CBackend backend(2);
  I2COutput::Options options(0, "Test I2C Device");
  options.pixel_count = 2;
  I2COutput output(m_uid, &backend, options);
  output.SetPersonality(I2COutput::PERS_P9813_INDIVIDUAL);
 ...
 */
void I2COutputTest::testCombinedP9813Control() {
  FakeI2CBackend backend(2);
  I2COutput::Options options(0, "Test I2C Device");
  options.pixel_count = 2;
  I2COutput output(m_uid, &backend, options);
  output.SetPersonality(I2COutput::PERS_P9813_COMBINED);
 ...
 */
void I2COutputTest::testIndividualAPA102Control() {
  // personality Individual APA102
  const uint16_t this_test_personality = I2COutput::PERS_APA102_INDIVIDUAL;
  // setup Backend
  FakeI2CBackend backend(2);
  I2COutput::Options options(0, "Test I2C Device");
  ...
  DmxBuffer buffer;
  // setup a pointer to the returned data (the fake I2C data stream)
  unsigned int length = 0;
  const uint8_t *data = NULL;
  ...
  output.WriteDMX(buffer);
  // get fake I2C data stream
  data = backend.GetData(0, &length);
  // this is the expected i2c data stream:
  ...
  // set StartAddress
  output.SetStartAddress(3);
  // values 1 & 2 should not be visible in I2C data stream
  buffer.SetFromString("1,2,3,4,5,6,7,8");
  output.WriteDMX(buffer);
  data = backend.GetData(0, &length);
  ...
  I2COutput::Options options1(1, "second I2C Device");
  // setup pixel_count to 2 (enough to test all cases)
  options1.pixel_count = 2;
  // setup I2COutput
  I2COutput output1(m_uid, &backend, options1);
  // set personality
  ...
  // setup I2COutput
  I2COutput output3(m_uid, &backend, options);
  // set personality
 ...
 */
void I2COutputTest::testCombinedAPA102Control() {
  const uint16_t this_test_personality = I2COutput::PERS_APA102_COMBINED;
  // setup Backend
  FakeI2CBackend backend(2);
  I2COutput::Options options(0, "Test I2C Device");
  // setup pixel_count to 2 (enough to test all cases)
  ...
  // simulate incoming dmx data with this buffer
  DmxBuffer buffer;
  // setup an pointer to the returned data (the fake I2C data stream)
  unsigned int length = 0;
  const uint8_t *data = NULL;
  ...
  // test for multiple ports
  // StartFrame is only allowed on first port.
  I2COutput::Options option1(1, "second I2C Device");
  // setup pixel_count to 2 (enough to test all cases)
  option1.pixel_count = 2;
  // setup I2COutput
  I2COutput output1(m_uid, &backend, option1);
  // set personality
  output1.SetPersonality(this_test_personality);
  // setup some 'DMX' data
  buffer.SetFromString("1, 10, 100");
 ...
 */
void I2COutputTest::testIndividualAPA102ControlPixelBrightness() {
  const uint16_t this_test_personality = I2COutput::PERS_APA102_PB_INDIVIDUAL;
  // setup Backend
  FakeI2CBackend backend(2);
  I2COutput::Options options(0, "Test I2C Device");
  // setup pixel_count to 2 (enough to test all cases)
 ...
 * Test DMX writes in the combined APA102 mode.
 */
void I2COutputTest::testCombinedAPA102ControlPixelBrightness() {
  const uint16_t this_test_personality = I2COutput::PERS_APA102_PB_COMBINED;
  // setup Backend
  FakeI2CBackend backend(2);
  I2COutput::Options options(0, "Test I2C Device");
  // setup pixel_count to 2 (enough to test all cases)
